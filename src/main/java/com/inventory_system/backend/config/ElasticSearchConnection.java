...
public class ElasticSearchConnection {
  ...
  public void createClient() {
    try {
      ...
      if (restClient == null)
        log.error("Elastic-> Error trying to create elasticsearh client");
      else
  ...
  public void insertDocument(Product product) {
    ...
    if (!words.isEmpty()) {
      ...
      String json = "{" +
              ...
              "\"id\":\"" + product.getId() + "\"," +
              "\"image\":\"" + (ObjectUtils.isEmpty(product.getImage()) ? "" : product.getImage()) + "\"" +
              "}";
  ...
  private List<ProductResponseDTO> getSuggestions(SearchResponse response) {
    ...
    for (CompletionSuggestion.Entry entry : entries) {
      for (CompletionSuggestion.Entry.Option option : entry.getOptions()) {
        ...
        productResponseDTO.setId(Integer.valueOf(option.getHit().getId()));
        productResponseDTO.setName((String) option.getHit().getSourceAsMap().get("name"));
        productResponseDTO.setImage((String) option.getHit().getSourceAsMap().get("image"));
  ...
  public void deleteDocument(Product product) {
    createClient();
    try {
      restClient.delete(new DeleteRequest(INDEX_NAME, String.valueOf(product.getId())), RequestOptions.DEFAULT);
    } catch (Exception e) {
      e.printStackTrace();
    }
    closeClient();
